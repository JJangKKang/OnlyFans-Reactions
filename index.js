        ...
        async generateAction(mesId, addInstr='', mode='add', postCount, useExistingProfile) {
            ...
            try {
                ...
                if (!this._cancelTokens.has(mesId)) {
                    ...
                    if (isCensored) {
                        // 검열 시 — 데이터 초기화 후 초기 생성 화면으로 복귀
                        this.settings.setData(mesId, null);
                        this.settings.clearMsgFingerprint(mesId);
                        this.settings.setCollapsed(mesId, false);
                        this.redraw(mesId);
                        window.OF_Reactions.Actions._toast(
                            mesId,
                            '⚠️ AI가 내용을 차단했습니다. 언어/설정을 변경 후 다시 생성해주세요.',
                            'neutral'
                        );
                    } else {
                        body.html(window.OF_Reactions.Templates.error(err.message || '생성 실패', mesId, false));
                    }
                }
