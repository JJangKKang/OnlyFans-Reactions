...
Input: ${jsonTexts}`;
        ...
        // Call API for translation — reuses Generator._callOpenAI / _callAnthropic for identical behavior
        async callApi(prompt, cfg) {
            ...
            // 번역 전용 API가 모두 설정된 경우 → 커스텀 API 사용
            if (url && key && model) {
                const gen = window.OF_Reactions.Controller.generator;
                if (!gen) throw new Error('Generator가 초기화되지 않았습니다.');
                return fmt === 'anthropic'
                    ? await gen._callAnthropic(url, key, model, prompt)
                    : await gen._callOpenAI(url, key, model, prompt);
            }

            // URL·키만 있고 모델이 없을 때 명확한 안내
            if ((url || key) && !model) {
                throw new Error('번역 모델명이 입력되지 않았습니다. 설정 > 번역 설정에서 모델명을 입력해주세요.');
            }

            // 아무것도 없으면 ST 기본 API 사용
            const ctx = SillyTavern.getContext();
            if (!ctx.generateRaw) throw new Error('번역 API 미설정. 설정 > 번역 설정에서 API를 입력하거나 ST 기본 API를 연결하세요.');
...
<div class="inline-drawer" id="of_reactions_ext_panel">
    ...
    <div class="inline-drawer-content">
        <div class="of-settings-panel">
            ...
            <button id="of_trans_test_btn" class="of-api-test-btn"><i class="fa-solid fa-flask"></i> 번역 API 연결 테스트</button>
            <div id="of_trans_test_result" class="of-api-test-result" style="display:none;"></div>
        </div>
    </div>
</div>`;
            ...
            $('#of_trans_test_btn').on('click', async function () {
                const result = document.getElementById('of_trans_test_result');
                result.style.display = 'block';
                result.className = 'of-api-test-result of-test-loading';
                result.textContent = '⏳ 번역 API 연결 테스트 중...';
                try {
                    const s2 = settings.get();
                    const testPrompt = `Translate to ${s2.translateTargetLang||'ko'}. Return ONLY a JSON array: ["Hello", "Thank you"]`;
                    const text = await window.OF_Reactions.Translator.callApi(testPrompt, s2);
                    result.className = 'of-api-test-result of-test-ok';
                    result.textContent = `✅ 번역 API 연결 성공! 응답: ${text.substring(0, 80)}`;
                } catch(e) {
                    result.className = 'of-api-test-result of-test-fail';
